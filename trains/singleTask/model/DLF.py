...
class DLF(nn.Module):
    ...
    def forward(self, text, audio, video):
        ...
        def _aux_of(mod):
            v = getattr(mod, "last_aux_loss", None)
            # 返回与当前设备一致的标量张量
            return v if isinstance(v, torch.Tensor) else proj_x_l.new_zeros(())

        moe_aux = proj_x_l.new_zeros(())  # 标量

        #disentanglement
        s_l = self.encoder_s_l(proj_x_l)
        moe_aux = moe_aux + _aux_of(self.encoder_s_l)
        s_v = self.encoder_s_v(proj_x_v)
        moe_aux = moe_aux + _aux_of(self.encoder_s_v)
        s_a = self.encoder_s_a(proj_x_a)
        moe_aux = moe_aux + _aux_of(self.encoder_s_a)
        ...
        moe_aux = moe_aux + _aux_of(self.encoder_c) * 3
       ...
       #  logits_a_low = self.out_layer_a_low(hs_proj_a_low)
        ...
        moe_aux = moe_aux + _aux_of(self.self_attentions_c_l)
        if type(c_l_att) == tuple:
            c_l_att = c_l_att[0]
        c_l_att = c_l_att[-1]

        c_v_att = self.self_attentions_c_v(c_v)
        moe_aux = moe_aux + _aux_of(self.self_attentions_c_v)
        if type(c_v_att) == tuple:
            c_v_att = c_v_att[0]
        c_v_att = c_v_att[-1]

        c_a_att = self.self_attentions_c_a(c_a)
        moe_aux = moe_aux + _aux_of(self.self_attentions_c_a)
        ...
        moe_aux = moe_aux + _aux_of(self.trans_l_mem)
        if type(h_ls) == tuple:
            h_ls = h_ls[0]
        ...
        moe_aux = moe_aux + _aux_of(self.trans_l_with_a)
        h_as = h_l_with_as
        ...
        moe_aux = moe_aux + _aux_of(self.trans_l_with_v)
        h_vs = h_l_with_vs
        h_vs = self.trans_v_mem(h_vs)
        moe_aux = moe_aux + _aux_of(self.trans_v_mem)
        if type(h_vs) == tuple:
            h_vs = h_vs[0]
