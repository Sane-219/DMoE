...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, layers, attn_dropout=0.0, relu_dropout=0.0, res_dropout=0.0,
                 embed_dropout=0.0, attn_mask=False, use_moe=True, moe_last_n_layers=3,
                 moe_num_experts=8, moe_top_k=2,
                 moe_capacity=1, moe_hidden_ratio=0.75,
        ...
        for layer in range(layers):
            use_moe_here = use_moe and (layer >= layers - moe_last_n_layers)
    ...
    def forward(self, x_in, x_in_k=None, x_in_v=None):
        ...
        aux_total = 0.0
        for layer in self.layers:
            if hasattr(layer, "last_aux_loss") and layer.last_aux_loss is not None:
                aux_total = aux_total + layer.last_aux_loss
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, embed_dim, num_heads=4, attn_dropout=0.1, relu_dropout=0.1, res_dropout=0.1,
                 attn_mask=False, use_moe=True, moe_num_experts=12, moe_top_k=2,
                 moe_capacity=1.75, moe_hidden_ratio=0.75, moe_expert_dropout=0.1):
        super().__init__()
        self.embed_dim = embed_dim
        ...
        if self.use_moe:
            # 原 FFN 隐层是 4*embed_dim，这里缩到一半以抵消 top-2
            moe_hidden = max(64, int(4 * self.embed_dim * moe_hidden_ratio))
            self.moe_ffn = MoEFFN(self.embed_dim, moe_hidden,
                                  num_experts=moe_num_experts,
                                  top_k=moe_top_k,
                                  capacity_factor=moe_capacity,
                                  noisy_gating=True,
                                  dropout=relu_dropout,  # 与原 relu_dropout 对齐
    ...
    def forward(self, x, x_k=None, x_v=None):  # Two Transformer layers
        ...
        if self.use_moe:
            x_ffn = self.moe_ffn(x)  # [T,B,D]
            # 记录本层 MoE 正则
            self.last_aux_loss = getattr(self.moe_ffn, "last_aux_loss", None)
        else:
            x_ffn = F.relu(self.fc1(x))
            x_ffn = F.dropout(x_ffn, p=self.relu_dropout, training=self.training)
            x_ffn = self.fc2(x_ffn)
            self.last_aux_loss = None
        x = F.dropout(x_ffn, p=self.res_dropout, training=self.training)
        x = residual + x
